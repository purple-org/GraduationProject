...
class accept_order extends StatefulWidget {
  final phone;
  final DateTime time;
  accept_order({this.phone,this.time});
  @override
...
class _order extends State<order> {
  ...
  Future getallorders() async {
    var formattedDate = DateFormat('yyyy-MM-dd').format(widget.time);
    var url = 'https://'+IP4+'/testlocalhost/PHP/day_order_accept.php';
    var ressponse = await http.post(url, body: {
      "phone": widget.phone,
      "date": formattedDate,
    });
...
class _new_order extends State<new_order> {
  ...
  Future delete (final id)async{
    var url = 'https://'+IP4+'/testlocalhost/PHP/deleteorder.php';
  ...
  Future accept (final id)async{
    var url = 'https://'+IP4+'/testlocalhost/PHP/acceptorder.php';
    var response = await http.post(url, body: {
      "id":id,
    });
    // String massage = json.decode(response.body);
